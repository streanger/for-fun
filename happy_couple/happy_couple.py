...
def check_position(file1, pos1, file2, pos2):
    ...
    # read first image
    file = 'th_view_02.png'
    stable = cv2.imread(os.path.join('lines_only', file1), cv2.IMREAD_UNCHANGED)
    # img = paste_image(stable, img, 400, 388)
    img = paste_image(stable, img, pos1[0], pos1[1])
    
    # read second image
    # second = 'th_view_03.png'
    loaded = cv2.imread(os.path.join('lines_only', file2), cv2.IMREAD_UNCHANGED)
    # pos_x, pos_y = 200, 200
    pos_x, pos_y = pos2
...
if __name__ == "__main__":
    ...
    if True:
        ...
        while True:
            ...
            commandNo += 1
            if commandNo > 1:
                winsound.PlaySound(os.path.join('sounds', 'smb_jump.wav'), winsound.SND_ALIAS | winsound.SND_ASYNC)
            ...
            if command == 'help':
                ...
                loaded = cv2.imread(os.path.join('lines_only', 'th_view_03.png'), cv2.IMREAD_UNCHANGED)
                blank = create_blank_image(heightMain, widthMain)
                ...
                blank = create_blank_image(heightMain, widthMain)
                img = draw_tv_background_and_backlight(blank)
                woman = cv2.imread(os.path.join('lines_only', 'waiting_woman.png'), cv2.IMREAD_UNCHANGED)
                ...
                # if 1:
                    ...
                    x_pos = int(x_pos)
                    y_pos = int(y_pos)
                    loaded = cv2.imread(os.path.join('lines_only', file), cv2.IMREAD_UNCHANGED)
