...
def resize_image(file_name, new_height, new_width):
    subDir = "RESIZED"
    image = cv2.imread(file_name)
    resized = cv2.resize(image, (new_width, new_height))
    #cut circle here
    b_channel, g_channel, r_channel = cv2.split(resized)
    alpha_channel = np.zeros((new_width, new_height,1), dtype='uint8')      #add alpha channel
...
if __name__ == "__main__":
    ...
    #file = args[0]
    file = "berk.png"
    status = resize_image(file, 280, 280)
    #files = get_files()
    #for file in files:
    #    status = resize_image(file, 80, 80)
    print("finished...")