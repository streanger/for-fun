'''copied from:
https://stackoverflow.com/questions/21241708/python-get-a-list-of-selected-files-in-explorer-windows-7?lq=1

can't handle arabic characters. Think of this:
...
def explorer_fileselection():
    data = {}
    try:
        clsid = '{9BA05972-F6A8-11CF-A442-00A0C90A8F39}' #Valid for IE as well!     # if won't work copy it to global
        address_1=""
        ...
        for window in range(shellwindows.Count):
            ...
            # print("Directory --> {}".format(window_dir))
            data[window_dir] = []
            if window_dir==address_1:
                ...
                # print("Directory --> {}, Files --> {}".format(window_dir, files))
                data[window_dir] = files
            files = []
            
    except:
        # possible error:
            # pywintypes.error: (1400, 'GetClassName', 'Nieprawidłowe dojście okna.')
        # print('error handled')
