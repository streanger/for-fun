...
def create_picture(data, shape, param1, param2, param3):
    ...
    for key, (x_pos, y_pos) in enumerate(data):
        ...
        # img[x_pos+centerX, y_pos+centerY] = (key*x_pos*y_pos*(math.tan((out)/100) + math.sin((out)/100))/1000000 % 255)   # nice
        img[x_pos+centerX, y_pos+centerY] = (key*x_pos*y_pos*(math.tan((out)/(100+param1)) + math.sin((out)/(100+param2)))/(x*10000 + 1000000) % 255)
...
if __name__ == "__main__":
    ...
    for x in range(50):
        # picture = create_picture(data, (size*2, size*2), x/5, 10.0-x/5, x)
        picture = create_picture(data, (size*2, size*2), 1, 1, x)
        out = np.repeat(np.repeat(picture, 2, axis=0), 2, axis=1)
        cv2.imwrite('image_{0:02d}.png'.format(x), out)
        print('image_{0:02d}.png created'.format(x))
