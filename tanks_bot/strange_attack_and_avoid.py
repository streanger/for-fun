        ...
        try:
            if data['type'] in ('bullet', 'npc'):
                ...
                if bullet[0] in ('left', 'right'):
                    if bullet[0] == 'left':
                        if bullet[1]['y'] in range(player_pos[1]-16, player_pos[1]+16):
                    ...
                    if bullet[0] == 'up':
                        if bullet[1]['x'] in range(player_pos[0]-16, player_pos[0]+16):
...
class Game(object):
    ...
    async def loop_game(self):
        ...
        if not self.first_tick:
            await self.send(action='greet', name='avoid')
        ...
        if self.start:
            ...
            # if not (self.player.counter % 45):
                # action=choice(['right', 'left'])
            print("\n\n\t\t\t\t\taction: '{}'\n\n".format(action))
            if action and self.player.counter > 15:
                if action == 'left':
                    ...
                    await self.send(action='set_speed', speed=2)            
                elif action == 'stop':
...
async def handle_client(loop):
    ...
    while True:
        ...
        if action:
            game.player.current_action = action
            await game.receive(data)
            continue
        ...
        print("\n\t\t\t\t\t\tbullet ray: {}".format(bullet_avoid))
